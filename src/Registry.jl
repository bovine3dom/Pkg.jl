...
update(regs::Vector{RegistrySpec} = Types.collect_registries(;clone_default=false)) =
    update(Context(), regs)
update(ctx::Context, regs::Vector{RegistrySpec} = Types.collect_registries(;clone_default=false)) =
